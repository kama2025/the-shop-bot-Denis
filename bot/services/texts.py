...
DEFAULT_TEXTS: dict[str, dict] = {
    ...
    "product_card": {
        ...
        "hint": "Доступно: {title}, {description}, {price_usd}, {price_rub}, {total}",
        "value": (
            "🧾 <b>{title}</b>\n\n"
            "💲 <b>Цена:</b> {price_usd} ({price_rub})\n\n"
            "{description}\n\n"
            "💰 К оплате: <b>{total}</b>\n"
            "<i>В сумму включён сервисный сбор.</i>"
        ),
    },
    "rate_unavailable": {
        "title": "Курс валют недоступен",
        "hint": "Показывается, когда курс ЦБ не получен ни разу",
        "value": (
            "⏳ Курс валют временно недоступен, поэтому цену в рублях "
            "показать не можем.\n"
            "Загляните через несколько минут — обычно это решается само."
        ),
    },
    "order_summary": {
        "title": "Оформление заказа",
        "hint": "Доступно: {order_id}, {title}, {subtotal}, {discount}, {total}, {promo}",
        "value": (
    ...
    "ask_credentials": {
        "title": "Просьба прислать логин и пароль",
        "hint": "Показывается сразу после оплаты. Доступно: {order_id}, {title}, {token}",
        "value": (
            "✅ <b>Оплата получена. Заказ #{order_id}</b>\n"
            "🎟 Токен: <code>{token}</code>\n\n"
            "📦 {title}\n\n"
            "🔑 Пришлите <b>логин и пароль</b> от аккаунта одним сообщением:\n"
            "первой строкой логин, второй — пароль.\n\n"
            "<i>Данные увидит только администратор, который выполняет заказ.</i>"
        ),
    },
    "credentials_accepted": {
        "title": "Реквизиты приняты",
        "hint": "Доступно: {order_id}, {title}, {token}",
        "value": (
            "🛠 <b>Заказ #{order_id} принят в работу</b>\n"
            "🎟 Токен: <code>{token}</code>\n\n"
            "📦 {title}\n\n"
            "Мы сообщим, когда всё будет готово. "
            "Номер заказа называйте по токену — так быстрее."
        ),
    },
    "order_done": {
        ...
        "value": (
            ...
            "🎟 Токен: <code>{token}</code>\n\n"
            "📦 {title}\n\n"
            "Спасибо! 🤍 Если что-то не так — напишите в поддержку и назовите токен."
        ),
    },
    "profile": {
        "title": "Профиль",
        "hint": "Доступно: {name}, {user_id}, {orders}, {spent}, {since}",
        "value": (
            "👤 <b>Профиль</b>\n\n"
    ...
    "info": {
        ...
        "value": (
            "ℹ️ <b>Информация</b>\n\n"
            "• Цены указаны в долларах, оплата — в рублях по курсу ЦБ.\n"
            "• После оплаты пришлите логин и пароль от аккаунта: "
            "первой строкой логин, второй — пароль.\n"
            "• В ответ придёт токен заказа — называйте его при обращении.\n"
    ...
    "terms": {
        ...
        "value": (
            "📄 <b>Условия</b>\n\n"
            "1. Оплачивается работа над вашим аккаунтом. Для её выполнения "
            "нужны логин и пароль — их видит только администратор, который "
            "выполняет заказ.\n"
            "2. Цена указана в долларах, к оплате — рубли по курсу ЦБ "
            "с сервисным сбором.\n"
            "3. Заказ считается выполненным после подтверждения администратором.\n"
            "4. Возврат оформляется администратором, деньги переводятся "
            "отдельно — внутреннего баланса у магазина нет.\n"
            "5. Продавец не несёт ответственности за нарушение покупателем "
            "правил сервисов, к которым относится заказ."
        ),
    },
    "paysupport": {
