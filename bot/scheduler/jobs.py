...
def setup(
    ...
    scheduler.add_job(
        ...
        # Допуск по умолчанию у APScheduler — одна секунда, а между сборкой
        # расписания и стартом планировщика проходит больше: бот успевает
        # подключиться к Telegram. Первый запуск просто пропускался с записью
        # «missed by 0:00:03» в журнале, и курс не приезжал до следующего часа.
