...
async def _edit_or_resend(
    ...
    # Правкой не обойтись — отправляем новое сообщение и только после этого
    # убираем старое.
    #
    # Порядок здесь важнее, чем кажется. Раньше старое сообщение удалялось
    # первым, «в расчёте» на то, что новое отправится. Когда отправка падала
    # (например, из-за неверного стиля кнопки, который Telegram не принимает),
    # экран у покупателя просто исчезал: старого нет, нового нет. Разрушающий
    # шаг не должен выполняться на предположении, что следующий удастся.
    sent = await _send(message, text, reply_markup, photo)
    try:
        await message.delete()
    except TelegramBadRequest:
        # Сообщение старше 48 часов удалить нельзя — не беда, новое уже есть.
        pass
    return sent
